...
def create_infrastructure():
    ...
    core = Node("core", cu=10, power_model=PowerModelNode(max_power=0.1, static_power=1.9e-3)) #based of malmodin (2020)
    access = Node("access", cu=10, power_model=PowerModelNode(max_power=0.27, static_power=1.2e-3)) #based of malmodin (2020)
    userdevice = Node("userdevice", cu=30, power_model=PowerModelNode(max_power=110, static_power=43.5)) # the value for max is for 50 inch tv the static power is average of a few values available on the internet.
...
def create_application(source_node: Node, sink_node: Node):
    ...
    source_task = SourceTask(cu=0.3, bound_node=source_node)
    processing_task = ProcessingTask(cu=0)
    sink_task = SinkTask(cu=0.1, bound_node=sink_node)
